...
- ✅ Fase 26b — SSE streaming (web/sse.py com authenticate_sse (header ou ?token=) + stream_dm_narration com producer thread + asyncio.Queue + format_sse wire format; rota POST /api/sessions/{sid}/stream retorna text/event-stream com eventos start/token/done/error; frontend com toggle "Modo streaming" que usa fetch + ReadableStream + TextDecoder para appendar tokens live; testes cobrem ordem dos eventos, empty token stream, error propagation, format helper; 7 testes SSE)
